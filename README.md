...
Each of these SmartNIC features a large Arria 10 FPGA, 4 GB of DDR4-SDRAM, 128MB of flash memory for FPGA configuration, one Mellanox NIC ASIC, one QSFP+ port for 50Gb Ethernet connectivity, three 8x PCIe Gen3 interfaces and several LEDs.  
...
- Several external headers.
  ...
  - One header for JTAG connection with standard Altera Blaster.
...
- The dedicated JTAG header doesn't work (??).  
- Find the purpose of following components.  
  ...
  - U22 (Level shifter with programmable I/O directions), unknown purpose, seems to communicate the FPGA with the Mellanox NIC ASIC.  
...
#### About the use of Quartus programmer and tools with these boards
